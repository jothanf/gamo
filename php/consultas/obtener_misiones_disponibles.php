...
if (isset($_GET['inquilino_id'])) {
    ...
    if ($resultado_avatar->num_rows > 0) {
        ...
        // Consultar misiones disponibles que no hayan sido asignadas al avatar
        $sql_misiones = "SELECT m.* FROM misiones m 
                         WHERE m.id NOT IN (
                            SELECT mision_id FROM mision_asignada 
                            WHERE avatar_id = ? AND estado != 'cancelada'
                         )
                         ORDER BY RAND() LIMIT 3"; // Limitamos a 3 misiones aleatorias
